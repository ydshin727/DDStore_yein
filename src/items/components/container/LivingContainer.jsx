...
const LivingContainer = () => {
  ...
  useEffect(() => {
    const bestFn = async () => {
      try {
        const res = await fetch(`${url}/items?category=living`);
  ...
  return (
    <div className="best">
      ...
      <h2><div className={`title ${showTitle ? 'show' : ''}`}>생활용품</div></h2>
            ...
            <ul>
              ...
              {pagedItems.map((el, idx) => {
                return (
                  <li key={el.id}>
                    <div className="top">
                      <Link to={`/items/living/detail/${el.id}`} className="item-link" onClick={() => window.scrollTo(0, 0)}>
